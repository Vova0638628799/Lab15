...
*/
struct student_data *recording_info_student(char *name_student, char *name_kurator, char *name_faculti, char *group_n)
{
...
struct student_arr *read_from_file(char *path_input_file)
{
	...
	for (unsigned int i = 0; i < n_str; i++) {
		...
		(*(arr_students->students + i)) = recording_info_student(name_student, name_kurator, name_faculti, group_n);
		...
		if (strcmp(budget_edu, "так") == 0) {
			(*(arr_students->students + i))->budget_edu = BUDGET_ON;
		} else {
			(*(arr_students->students + i))->budget_edu = BUDGET_OFF;
		}

		if (strcmp(name_building_cafedra, "ВК") == 0)
			(*(arr_students->students + i))->name_building_cafedra = BK;
		else if (strcmp(name_building_cafedra, "ГАК") == 0)
			(*(arr_students->students + i))->name_building_cafedra = GAK;
		else if (strcmp(name_building_cafedra, "У1") == 0)
			(*(arr_students->students + i))->name_building_cafedra = Y1;
		else
			(*(arr_students->students + i))->name_building_cafedra = Y2;
	}
	fclose(input_file);
...
struct student_arr *auto_generate_arr_student(unsigned int n_students)
{
	...
	for (unsigned int t = 0; t < n_students; t++) {
		...
		(*(arr_students->students + t)) = recording_info_student(name_student, name_kurator, name_faculti, group_n);
		if ((*(arr_students->students + t)) == NULL) {
			free_memory(arr_students);
			return NULL;
		}

		(*(arr_students->students + t))->budget_edu = (unsigned int)rand() % 2;
		(*(arr_students->students + t))->name_building_cafedra = (enum cafedra)rand() % 4;
